...
def mc_minimize(lattice, epochs=1, all=True):
    ...
    while moves < 2*lattice.N*epochs:
        ...
        if np.linalg.norm(x - x_old)/lattice.d > 1e-5:
            ...
            if moves%10 == 0:
                print("\tenergy = {}".format(lattice.energy()))
                f = lattice.plot_lattice(title=str(int(moves/10)), all=all)
                f.savefig("plots/{}.png".format(str(int(moves/10)).zfill(4)))
                plt.close()
            moves += 1
...
if __name__ == '__main__':
    ...
    mc_minimize(L, epochs=1000, all=False)
